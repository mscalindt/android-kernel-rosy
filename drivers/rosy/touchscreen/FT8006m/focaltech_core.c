...
#if FTS_GESTURE_EN
extern struct gesture_struct ft8006m_gesture_data;
...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "ft8006m_ts"
...
******************************************************************************/
struct i2c_client *ft8006m_i2c_client;
struct fts_ts_data *ft8006m_wq_data;
struct input_dev *ft8006m_input_dev;
extern char Lcm_name[HARDWARE_MAX_ITEM_LONGTH];

#if FTS_DEBUG_EN
int ft8006m_g_show_log = 1;
#else
int ft8006m_g_show_log = 0;
#endif
...
int ft8006m_ts_suspend(struct device *dev);
static int ft8006m_ts_resume(struct device *dev);
...
{
    /* wait tp stable */
    ft8006m_wait_tp_to_valid(client);
    /* recover TP charger state 0x8B */
    /* recover TP glove state 0xC0 */
    /* recover TP cover state 0xC1 */
    ft8006m_ex_mode_recovery(client);
    /* recover TP gesture state 0xD0 */
#if FTS_GESTURE_EN
    if (ft8006m_gesture_data.gesture_all_switch)
    ft8006m_gesture_recovery(client);
#endif
}


/*****************************************************************************
*  Name: ft8006m_reset_proc
...
{
    gpio_direction_output(ft8006m_wq_data->pdata->reset_gpio, 0);
    msleep(20);
    gpio_direction_output(ft8006m_wq_data->pdata->reset_gpio, 1);
    msleep(hdelayms);
...
void ft8006m_irq_disable(void)
{
    ...
    spin_lock_irqsave(&ft8006m_wq_data->irq_lock, irqflags);

    if (!ft8006m_wq_data->irq_disable)
    {
        disable_irq_nosync(ft8006m_wq_data->client->irq);
        ft8006m_wq_data->irq_disable = 1;
    }

    spin_unlock_irqrestore(&ft8006m_wq_data->irq_lock, irqflags);
}

/*****************************************************************************
*  Name: ft8006m_irq_enable
*  Brief: enable irq
*  Input:
*  Output:
*  Return:
...
void ft8006m_irq_enable(void)
{
    ...
    if (ft8006m_wq_data->irq_disable)
    {
        enable_irq(ft8006m_wq_data->client->irq);
        ft8006m_wq_data->irq_disable = 0;
    }
		...
		if (value >= MXT_INPUT_EVENT_START && value <= MXT_INPUT_EVENT_END) {
			if (value == MXT_INPUT_EVENT_WAKUP_MODE_ON) {
				ft8006m_gesture_data.gesture_all_switch = 1;
			       ...
			       ft8006m_gesture_data.gesture_all_switch = 0;
			}else {
			        ft8006m_gesture_data.gesture_all_switch = 0;
...
/*****************************************************************************
*  Name: ft8006m_input_dev_init
*  Brief: input dev init
*  Input:
*  Output:
*  Return:
*****************************************************************************/
static int ft8006m_input_dev_init(struct i2c_client *client, struct fts_ts_data *data,  struct input_dev *input_dev, struct fts_ts_platform_data *pdata)
{
    int  err, len;
...
static void fts_release_all_finger(void)
{
    ...
    mutex_lock(&ft8006m_wq_data->report_mutex);
#if FTS_MT_PROTOCOL_B_EN
    for (finger_count = 0; finger_count < ft8006m_wq_data->pdata->max_touch_number; finger_count++)
    {
        input_mt_slot(ft8006m_input_dev, finger_count);
        input_mt_report_slot_state(ft8006m_input_dev, MT_TOOL_FINGER, false);
    }
#else
    input_mt_sync(ft8006m_input_dev);
#endif
    input_report_key(ft8006m_input_dev, BTN_TOUCH, 0);
    input_sync(ft8006m_input_dev);
    mutex_unlock(&ft8006m_wq_data->report_mutex);
}
...
static int ft8006m_input_dev_report_key_event(struct ts_event *event, struct fts_ts_data *data)
{
    int i;
...
#if FTS_MT_PROTOCOL_B_EN
static int ft8006m_input_dev_report_b(struct ts_event *event, struct fts_ts_data *data)
{
    int i = 0;
    int uppoint = 0;
    int touchs = 0;
...
#else
static int ft8006m_input_dev_report_a(struct ts_event *event, struct fts_ts_data *data)
{
    int i = 0;
    int uppoint = 0;
    int touchs = 0;
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    if (ft8006m_gesture_data.gesture_all_switch)
    {
        ...
        if (data->suspended)
        {
            ft8006m_i2c_read_reg(data->client, FTS_REG_GESTURE_EN, &state);
            if (state == 1)
            {
                ft8006m_gesture_readdata(data->client);
                return 1;
            }
        }
    }
    ...
    buf[0] = 0x00;
    ret = ft8006m_i2c_read(data->client, buf, 1, buf, (3 + FTS_ONE_TCH_LEN));
    if (ret < 0)
    {
    ...
    {
        buf[9] = 0x09;
        ft8006m_i2c_read(data->client, buf+9, 1, buf+9, (event->point_num - 1) * FTS_ONE_TCH_LEN);
    }
#else
    ret = ft8006m_i2c_read(data->client, buf, 1, buf, POINT_READ_BUF);
    if (ret < 0)
    {
    ...
    ft8006m_point_report_check_queue_work();
#endif
...
static void fts_report_value(struct fts_ts_data *data)
{
    ...
    if (0 == ft8006m_input_dev_report_key_event(event, data))
    {
        return;
    }

#if FTS_MT_PROTOCOL_B_EN
    ft8006m_input_dev_report_b(event, data);
#else
    ft8006m_input_dev_report_a(event, data);
#endif
...
#if FTS_ESDCHECK_EN
    ft8006m_esdcheck_set_intr(1);
#endif

    ret = fts_read_touchdata(ft8006m_wq_data);

    if (ret == 0)
    {
        mutex_lock(&ft8006m_wq_data->report_mutex);
        fts_report_value(ft8006m_wq_data);
        mutex_unlock(&ft8006m_wq_data->report_mutex);
    }

#if FTS_ESDCHECK_EN
    ft8006m_esdcheck_set_intr(0);
#endif
                                ...
                                unsigned long event, void *data)
{
        ...
        fts_data && fts_data->client)
    {
        ...
        if (*blank == FB_BLANK_UNBLANK)
            ft8006m_ts_resume(&fts_data->client->dev);
        else if (*blank == FB_BLANK_POWERDOWN)
            ft8006m_ts_suspend(&fts_data->client->dev);
    }
...
static void fts_ts_early_suspend(struct early_suspend *handler)
{
    ...
    ft8006m_ts_suspend(&data->client->dev);
}
...
static void fts_ts_late_resume(struct early_suspend *handler)
{
    ...
    ft8006m_ts_resume(&data->client->dev);
}
#endif
#if 1
...
void hardwareinfo_set(void*drv_data)
{
	...
	u8 fw_ver;
    	...
    	ft8006m_i2c_read_reg(ft8006m_i2c_client, FTS_REG_VENDOR_ID, &vendor_id);
    	ft8006m_i2c_read_reg(ft8006m_i2c_client, FTS_REG_FW_VER, &fw_ver);
	ft8006m_i2c_read_reg(ft8006m_i2c_client, FTS_REG_CHIP_ID, &ic_type);
	...
	if (ic_type == TP_IC_FT8006M)
	{
		snprintf(ic_name, HARDWARE_MAX_ITEM_LONGTH, "FT8006M");
	}else{
...
{
	lcd_power_ctrl(ft8006m_wq_data, 0);
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ft8006m_wq_data = data;
    ft8006m_i2c_client = client;
    ft8006m_input_dev = input_dev;

    spin_lock_init(&ft8006m_wq_data->irq_lock);
    mutex_init(&ft8006m_wq_data->report_mutex);

    ft8006m_input_dev_init(client, data, input_dev, pdata);

    ft8006m_ctpm_get_upgrade_array();
...
ft8006m_reset_proc(200);
	...
	reg_addr = 0xA3;
	for (i = 0; i < 5; i++){
		err = ft8006m_i2c_read(client, &reg_addr, 1, &reg_value, 1);
		if (err < 0)
			msleep(5);
		else
			break;
    ...
    ft8006m_i2c_read_reg(client, FTS_REG_CHIP_ID, &reg_value);
...
#if FTS_APK_NODE_EN
    ft8006m_create_apk_debug_channel(client);
#endif

#if FTS_SYSFS_NODE_EN
    ft8006m_create_sysfs(client);
#endif

#if FTS_POINT_REPORT_CHECK_EN
    ft8006m_point_report_check_init();
#endif

    ft8006m_ex_mode_init(client);

#if FTS_GESTURE_EN
   ft8006m_gesture_init(input_dev, client);
#endif

#if FTS_ESDCHECK_EN
    ft8006m_esdcheck_init();
#endif
    ...
    FTS_INFO("tp_fw=0x%x\n", data->fw_ver[0]);
#if FTS_TEST_EN
    ft8006m_test_init(client);
#endif
#if FTS_LOCK_DOWN_INFO
    ft8006m_lockdown_init(client);
#endif

#if FTS_CAT_RAWDATA
    ft8006m_rawdata_init(client);
#endif
...
#if FTS_POINT_REPORT_CHECK_EN
    ft8006m_point_report_check_exit();
#endif

#if FTS_APK_NODE_EN
    ft8006m_release_apk_debug_channel();
#endif

#if FTS_SYSFS_NODE_EN
    ft8006m_remove_sysfs(client);
#endif

    ft8006m_ex_mode_exit(client);

#if FTS_AUTO_UPGRADE_EN
    cancel_work_sync(&ft8006m_fw_update_work);
#endif
...
#if FTS_TEST_EN
    ft8006m_test_exit(client);
#endif

#if FTS_ESDCHECK_EN
    ft8006m_esdcheck_exit();
#endif
...
int ft8006m_ts_suspend(struct device *dev)
{
    struct fts_ts_data *data = ft8006m_wq_data;
    int retval = 0, i = 0;
	...
	retval = ft8006m_gesture_suspend(data->client);
	if (retval == 0)
	{
	    /* Enter into gesture mode(suspend) */
	    retval = enable_irq_wake(ft8006m_wq_data->client->irq);
	    if (retval)
    ...
    for (; i < 5; i++){
    /* TP enter sleep mode */
    retval = ft8006m_i2c_write_reg(data->client, FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP_VALUE);
    if (retval < 0)
    {
...
EXPORT_SYMBOL(ft8006m_ts_suspend);
/*****************************************************************************
*  Name: fts_ts_resume
*  Brief:
*  Input:
...
static int ft8006m_ts_resume(struct device *dev)
{
...
#if FTS_GESTURE_EN
		if ((!(ft8006m_gesture_data.gesture_all_switch)) || panel_dead2tp){
				FTS_ERROR("%s:panel_dead2tp=%d", __func__, panel_dead2tp);
				panel_dead2tp = 0;
				lcd_power_ctrl(data, 1);
		}
    ...
    ft8006m_tp_state_recovery(data->client);

#if FTS_ESDCHECK_EN
    ft8006m_esdcheck_resume();
#endif

#if FTS_GESTURE_EN
    if (ft8006m_gesture_data.gesture_all_switch){
    if (ft8006m_gesture_resume(data->client) == 0)
    {
        int err;
