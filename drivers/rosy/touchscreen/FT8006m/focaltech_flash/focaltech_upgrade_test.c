...
*****************************************************************************/
struct wakeup_source ft8006m_ps_lock;
...
* Return: 0
***********************************************************************/
static int ft8006m_ctpm_auto_upgrade_pingpong(struct i2c_client *client)
{
    u8 uc_tp_fm_ver;
    int i_ret = 0;
...
int ft8006m_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    wakeup_source_init(&ft8006m_ps_lock, "tp_wakelock");

    __pm_stay_awake(&ft8006m_ps_lock);
    ...
    do
    {
        ...
        i_ret = ft8006m_ctpm_auto_upgrade_pingpong(client);
        if (i_ret == 0)
        {
    ...
    wakeup_source_trash(&ft8006m_ps_lock);
