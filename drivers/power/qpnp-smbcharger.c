...
void smbchg_set_calling_current(struct smbchg_chip *chip)
{
	...
	if ((chip->call_state == 0)) {
		if (usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP) {
			...
			vote(chip->usb_icl_votable, CALL_ICL_VOTER, true,
				d1_call_current_max);
		}
	} else {
		...
		if ((battery_temperature > chip->customize_cool_low_limit)
			...
			if ((chip->fcc_customize_cool_ma > 0)
				...
				vote(chip->fcc_votable,
					RESTRICTED_CHG_FCC_VOTER, true,
					chip->fcc_customize_cool_ma);
			}
		} else {
			if (chip->batt_cool) {
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	pr_smb(PR_STATUS,
		"plugged cable will call smbchg_set_calling_current \n");
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_CALL_STATE:
		...
		pr_smb(PR_STATUS,
			"calling will call smbchg_set_calling_current and will detect cable plugged chip->call_state =%d\n",
			chip->call_state);
		smbchg_set_calling_current(chip);
		break;
	default:
...
module_param_call(period_ms, set_period_ms, param_get_uint, &period_ms, 0644);
...
static void period_update(struct work_struct *work)
{
	...
	int usb_present, usb_current, vbus;
	int period = 500;
	union power_supply_propval prop = {0, };
	...
	if ((period_ms >= 500) || (abs(temp - old_temp) >= 1)
			|| (old_cap != cap)
			|| (old_status != status)
			|| (old_present != present)
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->monitor_charging_temp_5_work,
			  smbchg_monitor_charging_temp_5_work);
