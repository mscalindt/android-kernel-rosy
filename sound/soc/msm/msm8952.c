...
static void msm8952_ext_spk_control(u32 enable)
{
	...
	pr_debug("External speaker PAs: %s\n", enable ? "enabled" : "disabled");
}

static __inline int lineout_status_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
parse_mclk_freq:
	...
	spk_pa_gpio = of_get_named_gpio(pdev->dev.of_node, "ext-spk-amp-gpio",
					0);
	if (spk_pa_gpio < 0)
		...
		if (gpio_request_one(spk_pa_gpio, GPIOF_DIR_OUT, "spk_enable"))
			dev_err(&pdev->dev, "%s: spk_pa_gpio request failed\n",
				__func__);
	...
	if (headset_gpio < 0)
		...
		if (gpio_request_one(headset_gpio, GPIOF_DIR_OUT,
				"headset_enable"))
			dev_err(&pdev->dev, "%s: headset_gpio request failed\n",
				__func__);
